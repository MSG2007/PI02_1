...
2)Построение архитектуры модели. Создание сверточной архитектуры для извлечения признаков.
3)Конволюционный блок с регуляризацией (def _conv_block(x, filters, kernel_size, dropout_rate=0.3)).
4)Создание синтетического датасета. Для каждого изображения: Создаём пустое 416x416 изображение; Добавляем 1-3 цветных прямоугольника (Red, Green, Blue); Кодируем в грид 13x13: gt[i, j, 0:2] = (x_offset, y_offset)... 
